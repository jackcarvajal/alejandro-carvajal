 ...
 * 2. Cloudflare Dashboard → alejandrocadcam → Settings → Environment Variables
 *    → Agregar: RESEND_API_KEY = re_xxxxxxxxx
 *    → Agregar: FROM_EMAIL = Alejandro Carvajal CAD/CAM <tu@dominio.com>
 *       (si no tienes dominio usa: onboarding@resend.dev para pruebas)
 */

function htmlEmail(titulo, contenido, codigo) {
  return `<!DOCTYPE html>
<html lang="es">
...
<style>
  ...
  .hdr{background:linear-gradient(135deg,#1a1a2e,#0f1629);padding:28px 32px;text-align:center;}
  .hdr h1{margin:0;font-size:1.1rem;font-weight:900;letter-spacing:2px;color:#D4AF37;}
...
function email_nuevoCaso(emailCliente, codigo, servicio) {
  return enviarEmail(
    emailCliente,
    `✅ Caso recibido #${codigo} — Alejandro CAD/CAM`,
    htmlEmail('Caso recibido', `
      <div class="badge">✅ CASO RECIBIDO</div>
      <h2>¡Recibimos tu caso de diseño CAD!</h2>
      <p>Confirmamos que tu orden fue creada exitosamente. Verificaremos tu pago para comenzar.</p>
...
function email_pagoConfirmado(emailCliente, codigo) {
  return enviarEmail(
    ...
    htmlEmail('Pago confirmado', `
      <div class="badge">💳 PAGO CONFIRMADO</div>
      <h2>¡Tu pago fue verificado!</h2>
      <p>Confirmamos la recepción de tu pago. Ya iniciamos el diseño de tu caso.</p>
...
function email_stlListo(emailCliente, codigo) {
  return enviarEmail(
    emailCliente,
    `📦 STL listo para descargar — Caso #${codigo}`,
    htmlEmail('STL entregado', `
      <div class="badge">📦 STL LISTO</div>
      <h2>¡Tu diseño fue aprobado!</h2>
      <p>El archivo STL está disponible en tu portal para descargar y producir en tu equipo.</p>
      <p style="font-size:.8rem;color:#94a3b8;">El link de descarga es válido por 24 horas.</p>
    `, codigo)
  );
}
