...
(function initOneSignal() {
  if (ONESIGNAL_APP_ID === 'PENDIENTE') return;
    ...
    await OneSignal.init({
      ...
      promptOptions: {
        slidedown: {
          prompts: [{
            type: 'push',
            autoPrompt: false, // NO preguntar automáticamente
            text: {
  ...
  el.innerHTML = `
    <div style="background:rgba(0,210,255,.06);border:1px solid rgba(0,210,255,.2);border-radius:10px;padding:14px 18px;display:flex;align-items:center;gap:14px;margin-top:16px;">
      ...
      <div style="flex:1">
        ...
        <div style="font-size:.72rem;color:#94a3b8">Recibe alertas cuando tu diseño esté listo</div>
      </div>
 ...
 * Llamar desde admin-panel cuando el diseño esté listo
 */
async function pushAlCliente(externalUserId, titulo, mensaje, url) {
  if (ONESIGNAL_APP_ID === 'PENDIENTE') return false;
  try {
