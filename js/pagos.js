...
async function abrirCheckoutWompi({ monto, referencia, email, descripcion, onSuccess }) {
    ...
    if (PAGOS_CONFIG.wompi.modoEspera) {
        ...
        if (confirm(msg + '\n\n¿Abrir WhatsApp ahora?')) {
            ...
            window.open('https://wa.me/573219581949?text=' + ref, '_blank', 'noopener,noreferrer');
        }
        return;
    }
    ...
    // el monto del cliente se ignora. Sin firma NO se abre el checkout
    // (fail-closed): antes se continuaba sin firma, permitiendo pagar un monto
 ...
 *      NINGUNA verificación server-side contra la API de PayPal.
 * Antes de activar PayPal: Edge Function que capture con PAYPAL_CLIENT_SECRET
 * server-side, confirme con api.paypal.com que amount == precio_total del
 * pedido en la BD, y marque Pagado vía service-role. Ver PENDIENTES.md.
 */
