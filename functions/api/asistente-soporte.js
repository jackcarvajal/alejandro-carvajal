...
const ARTICULOS_URL = null;                                               // este sitio no tiene Centro de ayuda (todo va en CONOCIMIENTO_PUBLICO)
const ARTICULOS_CLIENTE = [];
const NEGOCIO_DESC = 'servicio de diseño dental CAD/CAM remoto (coronas, puentes, full arch, guías quirúrgicas y más) para laboratorios y clínicas en Colombia y otros países';
const WA_TEXTO = '+57 321 958 1949 (https://wa.me/573219581949), lunes a sábado de 8 a. m. a 7 p. m.';
const CONOCIMIENTO_PUBLICO = `
- Sitio: ${SITIO}. WhatsApp: ${WA_TEXTO}.
- Enviar un escáner: ${SITIO}/envia-tu-scanner — formatos STL, OBJ, PLY, DCM o ZIP; máximo 50 MB por archivo. Si pesa más: comprimirlo en ZIP o pegar un enlace de descarga (Drive, WeTransfer, Dropbox) en las notas.
- Hacer un pedido de diseño: ${SITIO}/flujo-diseno — se sube un ZIP con todo el caso (máximo 200 MB); si pesa más, dividirlo o compartir un enlace externo.
- Formatos que se trabajan: STL, OBJ, PLY, proyectos exocad (.constructionfile), 3Shape Communicate (.3oxz), exportaciones de iTero, Medit y Primescan, y DICOM para planificar guías quirúrgicas.
- Pagar un pedido: ${SITIO}/pagar. En Colombia: Nequi, Daviplata, Bancolombia, PSE o tarjeta; internacional: tarjeta, PayPal o Wise.
- Seguimiento de un caso: ${SITIO}/seguimiento-caso con el código del caso. Clientes con cuenta: ${SITIO}/app/login.html → portal del cliente.
- Contraseña olvidada: en ${SITIO}/app/login.html hay opción para restablecerla (llega un correo; revisar también spam).
- Si el diseño no llegó en el tiempo acordado: escribir por WhatsApp con el código del caso para darle prioridad.
- Cada diseño incluye 1 ronda de revisiones sin costo cuando el ajuste es por interpretación de la indicación clínica.
- Precios y cotizaciones: no los des; remite a ${SITIO}/calculadora-diseno o a WhatsApp.
