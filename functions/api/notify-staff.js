/**
 * Alejandro CAD/CAM — Aviso al staff por WhatsApp cuando entra un lead
 * POST /api/notify-staff
 * Body: { doctor, servicio, whatsapp, pais }
 *
 * Los destinatarios NO vienen del request (anti-abuso): salen de env vars.
 * Cada número debe registrarse en CallMeBot y tener su propia apikey.
 *
 * Env vars (configurar en Cloudflare Pages):
 *   STAFF_1_PHONE / STAFF_1_APIKEY   → Alejandro (573219581949)
 *   STAFF_2_PHONE / STAFF_2_APIKEY   → Esposa    (5212311034504)
...
export async function onRequestPost({ request, env }) {
  ...
  // Destinatarios desde env (nunca desde el request)
  const staff = [];
  for (let i = 1; i <= 3; i++) {
  ...
  // Rate limit: máx 20 avisos/hora por IP (evita floods)
  const ip = request.headers.get('CF-Connecting-IP') || 'unknown';
  const rlKey = new Request('https://rl.internal/notify-staff-ac_' + ip);
